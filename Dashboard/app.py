...
(Place Tea_Export_Master_2008_2024.csv and DateGlobal_Tea_Price.csv next to this file.)
"""
...
def load_data():
    dp = pd.read_csv(os.path.join(APP_DIR, "DateGlobal_Tea_Price.csv"))
    de = pd.read_csv(os.path.join(APP_DIR, "Tea_Export_Master_2008_2024.csv"))
...
def run_xgboost(df, steps=12):
    ...
    for t in range(tr, n):
        ...
        m.fit(X.iloc[:t], yv.iloc[:t]); pred.append(m.predict(X.iloc[t:t + 1])[0])
    idx = dx.index[tr:]
    ...
    final = xgb.XGBRegressor(n_estimators=100, learning_rate=0.05, max_depth=3, random_state=42).fit(X, yv)
